...
# Generate run name
RUN_NAME = "Breakout-v5__ppo-atari-breakout__1__1743888954"
...
if __name__ == "__main__":
    ...
    checkpoint_files = [f for f in os.listdir(f"{CHECKPOINT_DIR}/ALE/{RUN_NAME}") if f.endswith(".pt")]
    start_update = 1
    global_step = 0

    if checkpoint_files:
        latest_checkpoint = max(checkpoint_files, key=lambda x: int(x.split('_')[1].split('.')[0]))
        checkpoint_path = os.path.join(f"{CHECKPOINT_DIR}/ALE/{RUN_NAME}", latest_checkpoint)
