...
GYM_ID = "CartPole-v1"  # Changed from LunarLander to CartPole
NUM_ENVS = 16
CAPTURE_VIDEO = False

# Training parameters
TOTAL_TIMESTEPS = 5000000  # Reduced from 10M since CartPole is simpler
...
# Tracking parameters
TRACK = True
WANDB_PROJECT_NAME = "ppo-cartpole"  # Changed project name
WANDB_ENTITY = None
EXP_NAME = "ppo-cartpole"  # Changed experiment name
...
# Generate run name
RUN_NAME = f"CartPole-v1__{EXP_NAME}__{SEED}__{int(time.time())}"  # Updated run name
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Modified for CartPole - even simpler network than LunarLander
        # CartPole has only 4 state variables
    ...
    for update in range(start_update, num_updates + 1):
        ...
        for step in range(0, NUM_STEPS):
            ...
            if len(step_result) == 5:  # New Gymnasium API
                ...
                done = np.logical_or(terminated, truncated)  # Combine both signals into a single done flag
            else:  # Old Gym API
